...
yarn patch-commit <temp-folder> > packages/pm-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/pm-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/pm-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
