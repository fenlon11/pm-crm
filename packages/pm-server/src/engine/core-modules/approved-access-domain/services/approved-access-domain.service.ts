...
import { SendApprovedAccessDomainValidation } from 'pm-emails';
import { SettingsPath } from 'pm-shared/types';
import { getSettingsPath, isDefined } from 'pm-shared/utils';
...
export class ApprovedAccessDomainService {
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via Persistent Recruiter) <${this.twentyConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
