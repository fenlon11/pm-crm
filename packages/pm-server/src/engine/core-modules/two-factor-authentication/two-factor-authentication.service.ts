...
import { TwoFactorAuthenticationStrategy } from 'pm-shared/types';
...
export class TwoFactorAuthenticationService {
  ...
  async initiateStrategyConfiguration(
    ...
    if (
      ...
      const issuer = `Persistent Recruiter${workspaceDisplayName ? ` - ${workspaceDisplayName}` : ''}`;
    ...
    const { uri, context } = new TotpStrategy(
      ...
      userEmail,
      `Persistent Recruiter${workspaceDisplayName ? ` - ${workspaceDisplayName}` : ''}`,
    );
