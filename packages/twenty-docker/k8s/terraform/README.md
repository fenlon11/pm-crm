...
| <a name="input_twentycrm_server_data_mount_path"></a> [twentycrm\_server\_data\_mount\_path](#input\_twentycrm\_server\_data\_mount\_path) | TwentyCRM mount path for servers application data. Defaults to '/app/packages/pm-server/.local-storage'. | `string` | `"/app/packages/pm-server/.local-storage"` | no |
